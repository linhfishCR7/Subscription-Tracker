...
function initializePageState() {
  ...
  console.log('Onboarding page element:', onboardingPage ? 'Found' : 'Not found');
  ...
  if (currentUser) {
    ...
    // Ensure main app is properly visible and onboarding is hidden
    if (mainApp) {
      mainApp.classList.remove('hidden');
      mainApp.style.display = 'block';
      console.log('Main app visibility confirmed');
    }
    if (onboardingPage) {
      onboardingPage.classList.add('hidden');
      onboardingPage.style.display = 'none';
      console.log('Onboarding page hidden confirmed');
    }
  } else {
    // User is not logged in, show onboarding
    console.log('User not authenticated - showing onboarding page');
    showOnboardingPage();

    // Ensure onboarding is properly visible and main app is hidden
    if (onboardingPage) {
      onboardingPage.classList.remove('hidden');
      onboardingPage.style.display = 'block';
      console.log('Onboarding page visibility confirmed');
    }
    if (mainApp) {
      mainApp.classList.add('hidden');
      mainApp.style.display = 'none';
      console.log('Main app hidden confirmed');
    }
  }

  console.log('Page state initialization complete');
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  auth.onAuthStateChanged(async user => {
    ...
    if (user) {
      console.log('User authenticated - reinitializing page state for logged-in user');

      // Reinitialize page state to ensure proper visibility
      initializePageState();
      ...
      console.log('User not authenticated - reinitializing page state for logged-out user');
      ...
      console.log('Logged out state initialized successfully');
    }
  });
